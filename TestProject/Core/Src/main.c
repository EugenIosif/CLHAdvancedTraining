...
char buffer[100] = {0};
uint64_t dh_e = 5; // Generator (g)
uint64_t dh_n = 0xFFFFFFFFFFFFFFFFULL - 58; // Large 64-bit Prime (p)
uint64_t myPrivateIntermediary = 0x123456789ABCDEF0ULL; // Private Key (a)
    ...
    while(1)
    {
        ...
        else if(state == WAITINGFORSECRET)
        {
            ...
            if(uart_data_rx[0] == SECRETRECEIVED)
            {
                ...
                sharedSecret = simple_rsa_encrypt(received_dh_value, myPrivateIntermediary, dh_n);
        ...
        else if(state == NEGOTIATING)
        {
            ...
            uint32_t dh_value = simple_rsa_encrypt(dh_e, myPrivateIntermediary, dh_n);
...
void computeFunctionSignature128B(uint32_t function, uint16_t payload_len, uint8_t *outputSignature)
{
  ...
  rsa_encrypt(&hashOutput[0], functionSignature, d, n);
  memcpy(outputSignature, functionSignature, 8);
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (BspButtonState == BUTTON_PRESSED)
    {
      ...
      for(int i = 0; i < 8; i++)
      {
          printf("%02x", functionSignature[i]);
      }
      printf("\n\r");
    }
    /* USER CODE END WHILE */
